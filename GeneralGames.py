...
class GeneralGames():
    ...
    @classmethod
    def getgamegenres(cls, genre_ids: list):
        genre_ids = cls.formatids(genre_ids)
        count = 0
        genres = post('https://api.igdb.com/v4/genres', **{'headers': cls.headers,'data': f' fields name; where id = {genre_ids};'}).json()
        print(genres)
        for genre in genres:
    ...
    def getcover(cls, coverID) -> str:
        cover_link = str(post('https://api.igdb.com/v4/covers', **{'headers': cls.headers, 'data': f'fields url; where id = {coverID};'}).json()[0]['url']) #this will return a small image of the cover. Come back and modify to return a normal size later.
